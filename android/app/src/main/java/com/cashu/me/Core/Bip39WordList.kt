...
object Bip39WordList {
    private const val JOINED: String =
    ...
    /**
     * iOS `invalidMnemonicWords`: indices (into the normalized word sequence)
     * of entries that are not BIP-39 English words.
     */
    fun invalidWordIndices(phrase: String): List<Int> {
        val entered = normalize(phrase).split(' ').filter { it.isNotBlank() }
        return entered.withIndex().filter { (_, word) -> word !in words }.map { it.index }
    }
}

...
- [x] **[P1 · iOS → Android] Add scanning for a P2PK recipient key.** Android now opens the same scanner-first flow as iOS, including the opt-in “Lock to my key” shortcut and a paste shortcut only when the clipboard contains a valid key. Scans and quick fills use one validation path and the same invalid-key copy.

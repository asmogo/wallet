...
- [ ] **[P0 · iOS → Android] Require an explicit choice before switching to a missing custom key.** Android automatically generates a new identity when the user selects Custom Key without one; iOS intercepts the switch and asks for confirmation before generating. **Done when:** selecting Custom Key never creates or replaces an identity implicitly and asks the user to confirm generation (or import) before the signer changes.

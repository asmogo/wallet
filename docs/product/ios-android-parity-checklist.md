...
- [ ] **[P1 · iOS → Android] Surface wallet startup failures on the onboarding welcome screen.** iOS shows both the wallet manager’s startup error and errors raised by the current create action; Android’s wallet state contains `errorMessage`, but onboarding renders only its local create error. **Done when:** Android presents initialization failures in user-facing language with an appropriate retry or recovery action.

- [ ] **[P1 · iOS → Android] Validate a custom first-mint URL before staging it.** iOS requires a parseable URL with a host and shows “That doesn't look like a mint URL”; Android’s onboarding normalizer accepts any non-empty value after adding a scheme and gives no feedback when normalization fails. **Done when:** Android applies the shared mint-URL validator, rejects malformed hosts, and keeps a visible inline explanation until the input changes.

- [ ] **[P2 · iOS → Android] Collapse the hidden seed phrase into one accessibility action.** iOS removes the unrevealed word grid from the VoiceOver tree and exposes a single “Reveal seed phrase” action; Android leaves the numbered placeholder grid as descendants of the reveal target. **Done when:** TalkBack announces one reveal control while hidden and exposes the ordered words only after reveal.

- [ ] **[P1 · iOS → Android] Translate per-mint restore failures into wallet-facing copy.** iOS puts `userFacingWalletMessage` in each failed restore row; Android renders the thrown exception message directly. **Done when:** Android uses the shared wallet error translator, preserves the Retry action, and never exposes CDK/FFI implementation text.

## App shell and Wallet home

- [ ] **[P1 · iOS → Android] Honor the preferred primary balance unit on Wallet home.** Android always constructs the home balance with sats as the primary value, even when the user selected fiat; iOS uses `amountDisplayPrimary`. **Done when:** Android opens with the saved sats/fiat preference and offers the same primary/secondary flip behavior as iOS.
...
- [ ] **[P1 · Android → iOS] State the selected Cashu Request route explicitly.** Android tells the user whether it will pay from a compatible mint, use the Lightning fallback, add a requested mint, or top up the target mint; iOS relies on the mint row and CTA wording. **Done when:** iOS confirmation makes the chosen route equally explicit without duplicating information.
...
- [ ] **[P1 · Android → iOS] Allow deliberate manual P2PK key entry.** Android exposes an editable recipient-key field; iOS’s lock action is scanner-first and only exposes clipboard content when it is already valid. **Done when:** iOS offers an explicit paste/type route in addition to scan and “my key.”
...
- [ ] **[P2 · Android → iOS] Prefer the friendly mint name on receive success.** Android uses the selected mint’s name; iOS uses the mint host. **Done when:** both use friendly name with normalized-host fallback.
...
- [ ] **[P1 · iOS → Android] Explain all consequences of removing a Cashu Request row.** iOS says the QR and pending payment routing remain valid; Android only says received payments stay in the wallet. **Done when:** Android states both guarantees before deletion.
...
- [ ] **[P2 · Converge] Standardize mint action capitalization.** iOS uses “Add Mint,” “Discover Mints,” and “Paste URL from Clipboard”; Android mixes “Add mint,” “Discover mints,” and sentence case while also showing a title-cased discovery sheet heading. **Done when:** both use one agreed title/action capitalization rule.

- [ ] **[P1 · iOS → Android] Add pull-to-refresh to mint discovery.** iOS can explicitly restart discovery and reload missing previews; Android has no refresh gesture or retry action. **Done when:** Android users can restart discovery without closing and reopening the sheet.
...
- [ ] **[P2 · iOS → Android] Add contextual discovery no-result copy.** iOS distinguishes no mints found from no query matches and repeats the query; Android says “Listening on Nostr…” or “No matches” without a query-specific explanation. **Done when:** Android differentiates waiting, exhausted discovery, and filtered-zero states.

- [ ] **[P2 · Converge] Use one empty-discovery retry instruction.** iOS says “Pull down to retry,” which Android cannot currently fulfill; Android says that announcements will appear. **Done when:** both state an action that actually exists after the refresh work is implemented.
...
- [ ] **[P2 · iOS → Android] Hide empty payment-method rows.** iOS omits the section or empty direction; Android always shows Receive and Send with “None.” **Done when:** Android hides absent data instead of presenting empty capabilities as content.
...
- [ ] **[P2 · Converge] Standardize currency row avatars.** iOS uses country/region flags; Android uses currency symbols. **Done when:** product chooses one accessible visual system and both retain the ISO code and localized name in text.

## Settings — privacy and background behavior

- [ ] **[P1 · iOS → Android] Disable periodic invoice checks when incoming checks are off.** iOS enforces this dependency; Android leaves the child toggle interactive. **Done when:** Android disables it visually and semantically, and the stored combination cannot imply work that will never run.
...
- [ ] **[P1 · Android → iOS] Add a separate startup pending-token check preference.** Android distinguishes startup refresh from ongoing sent-token claim detection; iOS uses the sent-token setting for both. **Done when:** iOS offers the same privacy/background-work granularity or both platforms intentionally collapse to one documented rule.
...
- [ ] **[P2 · Converge] Publish one accurate crash-report privacy promise.** iOS promises no personal data, wallet addresses, or amounts; Android promises no screenshots/view hierarchy and no Sentry PII. **Done when:** the implementation is verified and both platforms display the same complete, accurate statement.
...
- [ ] **[P0 · iOS → Android] Guard the switch to a custom-key signer.** iOS prompts to generate/import when no custom private key exists; Android can attempt the switch and silently fail. **Done when:** Android never leaves the user selecting a signer with no usable key.
...
- [ ] **[P1 · iOS → Android] Add paste/clear affordances and explanatory copy to nsec import.** iOS explains the purpose and provides a dedicated paste/clear control; Android shows a bare dialog field. **Done when:** Android supports explicit clipboard input, clear, validation, and the same consequence text.

- [ ] **[P0 · Converge] Make identity-replacement warnings complete.** iOS mentions that the Lightning address changes; Android mentions messages and contacts. **Done when:** both warnings cover Lightning address, Nostr identity/apps/messages, and replacement of the old key before a destructive confirmation.
...
- [ ] **[P2 · iOS → Android] Add an explicit Paste/Clear control to P2PK key import.** iOS provides one; Android relies on the text field’s system edit menu. **Done when:** Android offers the same discoverable clipboard and clear actions.
...
- [ ] **[P2 · Converge] Standardize mint identity display.** Across list rows, selectors, confirmations, receipts, and details, the apps alternate among friendly name, full URL, host, and shortened URL. **Done when:** both follow one hierarchy: friendly name, normalized host fallback, full URL only for copy/share/detail.

- [ ] **[P2 · Converge] Standardize zero-fee wording.** The apps alternate among “Free,” “No fee,” and “0 sat.” **Done when:** review, confirmation, receipt, and History use the agreed term for the same condition.

- [ ] **[P2 · Converge] Standardize success-title case and punctuation.** The same completed receive/payment appears as “Payment Received!”, “Payment received,” “Payment Sent!”, and “Payment sent.” **Done when:** every live success and historical state follows one shared copy table.
...
- [ ] **[P2 · Converge] Keep copy feedback visually and semantically consistent.** Some actions change button text, some swap to a green check, some rely on the system clipboard bubble, and reset times differ. **Done when:** each platform uses its native feedback pattern consistently, announces success, and keeps equivalent timing across matched actions.

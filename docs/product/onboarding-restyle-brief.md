...
| `restoreInput` | *(rewritten 2026-08-08, revised same day after device review)* Word-by-word entry: a 12-tick progress rail, one card holding the current word behind up to two empty ghost cards, and one chip row directly under the card that is three-state — the **paste chip** while nothing is entered (paste is the most common way in, so it takes the spot the eye is on), up to three BIP-39 completions while typing, reserved space otherwise. Helper/notice line under the chips, inside the card's column so it can never land in the scroll fade. The rail scrubs: press-and-hold then drag runs through the words live, one haptic tick per word; quick taps still jump. Functional, and the **only step that focuses on arrival** — see DESIGN.md §6. A checksum failure (which names no single word) swaps the card for a review grid of all twelve, each tappable back into the field. |

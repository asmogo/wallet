...
| **RestoreInput** | Back → Welcome (matching iOS); "Restore wallet." header; word-by-word entry — 12-tick progress rail, one word card behind up to two empty ghost cards, up to three BIP-39 completion chips, helper/notice line. Focuses on arrival. Checksum failure swaps the card for a tappable review grid of all twelve | "Continue" (onboarding) / "Next" (Settings), disabled until 12 valid words with a good checksum; "Paste seed phrase" ghost link while nothing is entered |
